...
| `useEffect` 안에서 데이터 패칭만 수행 | TanStack Query 또는 SWR 사용 |
| `useEffect` 의존성 배열 누락 | 반드시 의존성 명시, 빈 배열이면 의도를 확인 |
| 배열 렌더링 시 `key`에 index 사용 | 고유 ID를 key로 사용 |
| state를 직접 변경 (`state.push()`) | 새 배열/객체를 만들어 setState |
| `console.log` 배포 코드에 잔류 | 개발 중에만 사용, 배포 전 제거 |
| `dangerouslySetInnerHTML` 사용 | 사용 금지. 불가피 시 DOMPurify로 새니타이즈 |

---

## 11. 보안 규칙

- API 키, 토큰 등 민감 정보를 클라이언트 코드에 직접 넣지 않는다. 환경변수(`VITE_`, `REACT_APP_`)로 관리한다.
- 그래도 클라이언트 환경변수는 브라우저에 노출됨을 인지한다. 진짜 비밀값은 백엔드에서만 사용한다.
- 사용자 입력을 HTML에 직접 삽입하지 않는다. (XSS 방지)
- 외부 API 호출 시 에러 응답의 민감 정보를 사용자에게 그대로 노출하지 않는다.
- `.env.local`을 `.gitignore`에 반드시 추가한다.
