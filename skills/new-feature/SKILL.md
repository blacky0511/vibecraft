...
name: new-feature
description: |
  사용자가 기능 추가/구현/개발/수정/변경을 요청할 때 반드시 이 스킬을 먼저 호출하라.
  "만들어줘", "추가해줘", "구현해줘", "수정해줘", "변경해줘", "개선해줘", "바꿔줘" 등의
  요청이 있으면 코드를 바로 작성하지 말고 이 스킬을 호출하여 작업 크기(S/M/L)를
  판단한 뒤 적절한 워크플로우를 실행하라. 이 스킬 없이 바로 코드를 작성하면
  계획 없는 구현으로 품질이 저하되고 재작업이 발생한다.
  Triggers: 만들어줘, 추가해줘, 구현해줘, 수정해줘, 변경해줘, 개선해줘, 바꿔줘, 기능, feature, implement, create, build, modify, update
---
