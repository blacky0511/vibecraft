...
- `console.log`는 개발 중에만 사용하고, 배포 전에 제거한다.
- 환경변수는 `.env.local`에 저장하고, 클라이언트에서 사용할 변수만 `NEXT_PUBLIC_` 접두사를 붙인다.
- 이미지는 `next/image`의 `<Image>` 컴포넌트를 사용한다. `<img>` 태그 직접 사용을 피한다.
- 링크는 `next/link`의 `<Link>` 컴포넌트를 사용한다. `<a>` 태그 직접 사용을 피한다.
- `app/` 디렉토리 내 파일은 기본적으로 서버 컴포넌트임을 항상 인지한다.
