...
코드 변경이 발생하면 **항상** 호출되어 테스트를 작성한다. 테스트 없는 코드는 완성된 코드가 아니다.

> 어떤 코드 변경이든, 크기에 상관없이, 반드시 테스트가 함께 작성되어야 한다. (CLAUDE.md AI 행동 규칙 참고)

---

## TDD 절차

> TDD 사이클(Red→Green→Refactor)의 상세 규칙과 예외 조건은 **CLAUDE.md AI 행동 규칙**을 참고한다.
